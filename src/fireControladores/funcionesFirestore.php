...
class funcionesFirestore{
    ...
    public function addd(string $name)
    {
        $this->db->collection($this->name)->document($name)->add($data);
    }


   //al fin me salio el edo
   public function editarOK(string $nombreDELcoso, array $data = [])
    {
        try {
            $this->db->collection('mensajes')->document($nombreDELcoso)->update($data);
            // $this->db->collection('mensajes')->document('7c9d4f79654b4c3bb07c')->update($data);
            return true;
        } catch (Exception $ex) {
            return $ex->getMessage();
        }        
